...
public class GUI implements ActionListener
{
    ...
    public GUI()
    {
        ...
        rL = new ReinforcementLearning(w);
        //states = rL.readSavedStates();
    ...
    {
        /*
        int x = w.getPlayerX(), y = w.getPlayerY();
        Location loc = new Location(x,y);
        boolean[] percepts ={w.hasBreeze(x, y), w.hasStench(x, y), w.hasGlitter(x, y),
                            w.hasPit(x, y),w.hasWumpus(x, y), w.isUnknown(x, y),
                            w.wumpusAlive(), w.hasArrow()};
        Location[] neighs = new Location[4];
        Location up = loc.neighbourUp();    neighs[0] = up;
        Location right = loc.neighbourRight();  neighs[1] = right;
        Location down = loc.neighbourDown();    neighs[2] = down;
        Location left = loc.neighbourLeft();   neighs[3] = left;
        int[] neighbours = new int[4];
        for(int i = 0; i<4; i++){
            Location n = neighs[i];
            if (w.isValidPosition(n.x, n.y)){
                if (w.isUnknown(n.x, n.y)) neighbours[i] = State.N_UNKNOWN;
                else if (w.hasPit(n.x, n.y)) neighbours[i] = State.N_PIT;
                else if (w.hasWumpus(n.x, n.y)){ neighbours[i] = State.N_WUMPUS;
                } else {neighbours[i] = State.N_SAFE;}
            }
            else neighbours[i] = State.N_WALL;
        }
        State state;
        try {
